...
def download_media(url, quality, platform, media_type):
    ...
    # ✅ Fix: Corrected postprocessor usage
    postprocessors = []
    if media_type == "Audio Only":
        postprocessors.append({
            'key': 'FFmpegExtractAudio',
            'preferredcodec': 'mp3',  # ✅ Corrected audio conversion
            'preferredquality': '192'
        })
    else:
        postprocessors.append({
            'key': 'FFmpegVideoRemuxer',
            'preferedformat': 'mp4'  # ✅ Fixed spelling mistake ('preferredformat' → 'preferedformat')
        })
